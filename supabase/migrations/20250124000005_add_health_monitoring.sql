-- ═══════════════════════════════════════════════════════════════════════════
-- Migration: Health Monitoring & Alerts
-- ═══════════════════════════════════════════════════════════════════════════
--
-- Sistema de monitoramento de saúde e alertas para Sofia
-- Detecta problemas proativamente e gera alertas
--
-- ═══════════════════════════════════════════════════════════════════════════
...
CREATE TABLE IF NOT EXISTS health_checks (
  ...
  metric_type TEXT NOT NULL, -- 'error_rate', 'latency', 'lead_creation', 'abandonment', 'rag_performance'
...
CREATE INDEX idx_health_checks_type ON health_checks(metric_type);
...
CREATE TABLE IF NOT EXISTS alerts (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  alert_type TEXT NOT NULL, -- 'high_error_rate', 'high_latency', 'low_conversion', 'high_abandonment'
...
-- ═══════════════════════════════════════════════════════════════════════════
-- 3. FUNCTION: Obter Métricas de Saúde (últimas 24h)
-- ═══════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  WITH metrics AS (
    SELECT
      ...
      -- P95 latência
      ROUND(PERCENTILE_CONT(0.95) WITHIN GROUP (ORDER BY (ae.metadata->>'response_time_ms')::NUMERIC), 0) as p95_lat
...
COMMENT ON FUNCTION get_health_metrics IS 'Retorna métricas de saúde do sistema nas últimas 24h';

-- ═══════════════════════════════════════════════════════════════════════════
-- 4. FUNCTION: Executar Health Check e Gerar Alertas
-- ═══════════════════════════════════════════════════════════════════════════
...
BEGIN
  -- 1. Verificar Error Rate (threshold: 5%)
  SELECT
    ROUND(
      (COUNT(*) FILTER (WHERE (ae.metadata->>'error')::BOOLEAN = true)::NUMERIC
        / NULLIF(COUNT(*), 0)::NUMERIC) * 100,
  ...
  -- 2. Verificar Latência (threshold: 4000ms)
  SELECT
    ROUND(AVG((ae.metadata->>'response_time_ms')::NUMERIC), 0) INTO v_avg_latency
  FROM analytics_events ae
  WHERE ae.event_type = 'message_sent'
    AND ae.created_at >= NOW() - INTERVAL '1 hour';
  ...
  -- 3. Verificar Taxa de Conversão (threshold: < 10%)
  SELECT
    ROUND(
      (COUNT(DISTINCT l.id)::NUMERIC / NULLIF(COUNT(DISTINCT c.id), 0)::NUMERIC) * 100,
      2
  ...
  -- 4. Verificar RAG Performance (threshold: > 30% sem chunks)
  SELECT
    ROUND(
...
COMMENT ON FUNCTION run_health_check IS 'Executa verificação de saúde e gera alertas se necessário';
...
COMMENT ON FUNCTION get_unresolved_alerts IS 'Lista todos os alertas não resolvidos, ordenados por severidade e idade';
