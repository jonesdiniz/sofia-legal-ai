-- ═══════════════════════════════════════════════════════════════════════════
-- Migration: Health Monitoring & Alerts (CORRIGIDO)
-- ═══════════════════════════════════════════════════════════════════════════
--
...
-- ═══════════════════════════════════════════════════════════════════════════
-- 3. FUNCTION: Obter Métricas de Saúde (CORRIGIDO)
-- ═══════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  WITH metrics AS (
    SELECT
      -- Uptime (CORRIGIDO: cast para NUMERIC)
      ROUND(
...
-- ═══════════════════════════════════════════════════════════════════════════
-- 4. FUNCTION: Executar Health Check (CORRIGIDO)
-- ═══════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  v_error_rate := COALESCE(v_error_rate, 0);
  ...
  SELECT
    ROUND(
      (COUNT(DISTINCT l.id)::NUMERIC / NULLIF(COUNT(DISTINCT c.id)::NUMERIC, 0)) * 100,
      2
  ...
  v_conversion_rate := COALESCE(v_conversion_rate, 0);
  ...
  v_rag_performance := COALESCE(v_rag_performance, 0);
