-- ═══════════════════════════════════════════════════════════════════════════
-- Migration: Health Monitoring & Alerts (VERSÃO FINAL CORRIGIDA)
-- ═══════════════════════════════════════════════════════════════════════════
--
-- Sistema de monitoramento de saúde e alertas para Sofia
-- Detecta problemas proativamente e gera alertas
--
-- CORREÇÃO: Todos os casts para NUMERIC agora são explícitos e corretos
--
-- ═══════════════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════════════
-- 3. FUNCTION: Obter Métricas de Saúde (CORREÇÃO DEFINITIVA)
-- ═══════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  WITH metrics AS (
    SELECT
      -- Uptime (CORRIGIDO COM CAST EXPLÍCITO)
      ROUND(
      ...
      -- Error rate (CORRIGIDO COM CAST EXPLÍCITO)
      ROUND(
      ...
      -- Latência média (CORRIGIDO: CAST após AVG)
      ROUND((AVG((ae.metadata->>'response_time_ms')::NUMERIC))::NUMERIC, 0) as avg_lat,

      -- P95 latência (CORRIGIDO: CAST após PERCENTILE_CONT)
...
-- ═══════════════════════════════════════════════════════════════════════════
-- 4. FUNCTION: Executar Health Check (CORREÇÃO DEFINITIVA)
-- ═══════════════════════════════════════════════════════════════════════════
...
BEGIN
  -- 1. Verificar Error Rate (CORRIGIDO)
  SELECT
    ROUND(
      (COUNT(*) FILTER (WHERE (ae.metadata->>'error')::BOOLEAN = true)::NUMERIC
        / NULLIF(COUNT(*)::NUMERIC, 0)) * 100::NUMERIC,
  ...
  -- 2. Verificar Latência (CORRIGIDO)
  SELECT
  ...
  -- 3. Verificar Taxa de Conversão (CORRIGIDO)
  SELECT
    ROUND(
  ...
  -- 4. Verificar RAG Performance (CORRIGIDO)
  SELECT
    ROUND(
